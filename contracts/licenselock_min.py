...
PROTOCOL_FEE_BPS: u256 = u256(250)
@allow_storage
@dataclass
class FileEvidence:
    path: str
...
class LicenseLock(gl.Contract):
    ...
    def _is_immutable_commit_sha(commit: str) -> bool:
        ...
        if not re.fullmatch('[0-9a-fA-F]{40}', c):
            return False
        return c.lower() != '0' * 40
    def __init__(self):
    ...
    def create_claim(self, repo: str, commit: str, recipient: Address, claim_type: str, allowed_licenses: list[str], target_directory: str='') -> str:
    ...
    def get_claim(self, claim_id: str) -> dict:
        ...
        if claim.result_json:
            try:
                ...
                pass
        return {'id': claim.id, 'repo': claim.repo, 'commit': claim.commit, 'recipient': str(claim.recipient), 'funder': str(claim.funder), 'amount': int(claim.amount), 'claim_type': claim.claim_type, 'allowed_licenses_json': claim.allowed_licenses_json, 'state': claim.state, 'outcome': outcome, 'result_json': result_obj, 'target_directory': getattr(claim, 'target_directory', '')}
    @gl.public.view
    ...
    def cancel_claim(self, claim_id: str) -> None:
        ...
        claim.result_json = json.dumps({'outcome': 'CANCELED', 'reason': 'Claim canceled by funder prior to resolution. Escrow refunded in full.', 'files': []})
    ...
    def resolve(self, claim_id: str) -> None:
        ...
        def fetch_file(repo: str, commit: str, path: str) -> dict:
            ...
            for attempt in range(max_retries):
                ...
                match = re.search('SPDX-License-Identifier:\\s*([A-Za-z0-9\\.\\-\\+]+)', body, re.IGNORECASE)
                if match:
        ...
        def check_declared_license_copyleft(text: str) -> tuple[bool, str]:
            if not text:
                return (False, '')
            spdx_match = re.search('SPDX-License-Identifier:\\s*([A-Za-z0-9\\.\\-\\+]+)', text, re.IGNORECASE)
            if spdx_match:
                found_spdx = spdx_match.group(1).upper()
                if is_copyleft_identifier(found_spdx):
                    return (True, spdx_match.group(0))
            gnu_match = re.search('GNU\\s+(?:AFFERO\\s+|LESSER\\s+|LIBRARY\\s+)?GENERAL\\s+PUBLIC\\s+LICENSE', text, re.IGNORECASE)
            if gnu_match:
                return (True, gnu_match.group(0))
            mpl_match = re.search('MOZILLA\\s+PUBLIC\\s+LICENSE', text, re.IGNORECASE)
            if mpl_match:
        ...
        def parse_manifest_declared_license(manifest_path: str, raw_text: str) -> str:
            ...
            if manifest_path.endswith('package.json'):
                ...
                match = re.search('^\\s*license\\s*=\\s*["\\\']([^"\\\']+)["\\\']', raw_text, re.MULTILINE | re.IGNORECASE)
                if match:
                    return match.group(1).strip()
            return ''
        LICENSE_CANDIDATES = ['LICENSE', 'license.md', 'LICENSE.md', 'LICENSE.txt', 'license', 'COPYING', 'LICENSE.rst', 'LICENSE-MIT', 'LICENSE-APACHE']
        README_CANDIDATES = ['README.md', 'readme.md', 'README.rst', 'README.txt', 'README']
        MANIFEST_CANDIDATES = ['package.json', 'Cargo.toml', 'pyproject.toml']
        def evaluate_repository(include_raw: bool=False) -> dict:
            if target_claim_type == 'SPDX_MATCH':
                ...
                res = {'outcome': outcome, 'reason': reason, 'readme_path': readme_res['path'], 'license_path': license_res['path'], 'readme_excerpt': str(readme_res['excerpt']), 'license_excerpt': str(license_res['excerpt']), 'readme_spdx': readme_res['spdx'], 'license_spdx': license_res['spdx'], 'files': [{'path': readme_res['path'], 'excerpt': str(readme_res['excerpt']), 'status': readme_res['status']}, {'path': license_res['path'], 'excerpt': str(license_res['excerpt']), 'status': license_res['status']}]}
                if include_raw:
                ...
                res = {'outcome': outcome, 'reason': reason, 'license_path': license_res['path'], 'license_excerpt': str(license_res['excerpt']), 'license_spdx': license_res['spdx'], 'files': [{'path': license_res['path'], 'excerpt': str(license_res['excerpt']), 'status': license_res['status']}]}
                if include_raw:
                    res['license_raw'] = license_res.get('raw', '')
                return res
                ...
                if license_res['status'] != 'MISSING':
                    files_arr.append({'path': license_res['path'], 'excerpt': str(license_res['excerpt']), 'status': license_res['status']})
                files_arr.extend(manifest_evidence_files)
                res = {'outcome': outcome, 'reason': reason, 'license_path': license_res['path'], 'license_excerpt': str(license_res['excerpt']), 'manifest_evidence': manifest_evidence_files, 'files': files_arr}
                if include_raw:
                    res['license_raw'] = license_res.get('raw', '')
        ...
        result_dict = {'outcome': result_data['outcome'], 'reason': result_data.get('reason', ''), 'files': files_arr}
